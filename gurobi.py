...
class Gurobi:
 ...
 """ def solve1(self):
        ...
        # c1
        for i in self.prob.i:
            model.addConstr(quicksum(x_ik[i, k] for k in self.prob.k) == 1)
...
"""
    ...
    def solve(self):
        ...
        model.addConstrs(z_ijkl[i, j, k, l] <= x_ik[i, k] for i in self.prob.i for j in self.prob.i for k in self.prob.k for l in self.prob.k)

        # c3

        model.addConstrs(z_ijkl[i, j, k, l] <= x_ik[j, l] for i in self.prob.i for j in self.prob.i for k in self.prob.k for l in self.prob.k)

        # c4

        model.addConstrs(x_ik[i, k] + x_ik[j, l] - 1 <= z_ijkl[i, j, k, l] for i in self.prob.i for j in self.prob.i for k in self.prob.k for l in self.prob.k)
        ...
        # c7 - BIG M 1
        model.addConstrs((c_i[i] + self.prob.d_i.loc[i, 'd']) - c_i[j] + y_ij[i, j] * self.M >= 0 for i in self.prob.i for j in self.prob.i)

        # c8 - BIG M 2

        model.addConstrs((c_i[i] + self.prob.d_i.loc[i, 'd']) - c_i[j] - (1 - y_ij[i, j]) * self.M <= 0 for i in self.prob.i for j in self.prob.i)
